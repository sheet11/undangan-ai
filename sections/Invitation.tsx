...
export default function Invitation({ initialWedding }: { initialWedding?: Wedding }) {
  ...
  const sectionNodes: Record<string, ReactNode> = {
    ...
    gallery: preweddingImages.length > 0 ? <section key="gallery" className={`${theme.card} py-24`}><Container><SectionTitle eyebrow="Prewedding" title="Momen sebelum hari bahagia" /><div className="relative mt-12"><div ref={galleryScrollRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{preweddingImages.map((image, index) => <button key={`${image}-${index}`} type="button" onClick={() => openLightbox(index)} className={`relative w-[78%] shrink-0 snap-center overflow-hidden rounded-2xl border text-left transition hover:opacity-90 sm:w-[46%] lg:w-[31%] ${isSecondTheme ? "border-[#d8c4a2] bg-[#f6eee1]" : "border-[#e5dccb] bg-[#f7f4ed]"}`} aria-label={`Buka foto prewedding ${index + 1}`}><Image src={image} alt={`Prewedding ${index + 1}`} width={600} height={800} className="h-80 w-full object-cover" /></button>)}</div>{preweddingImages.length > 1 && <><button type="button" onClick={() => scrollGallery("left")} aria-label="Geser galeri ke kiri" className="absolute left-0 top-1/2 hidden -translate-x-4 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 p-2 text-[#23382f] shadow-md transition hover:bg-white sm:flex"><ChevronLeft size={20} /></button><button type="button" onClick={() => scrollGallery("right")} aria-label="Geser galeri ke kanan" className="absolute right-0 top-1/2 hidden translate-x-4 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 p-2 text-[#23382f] shadow-md transition hover:bg-white sm:flex"><ChevronRight size={20} /></button></>}</div></Container></section> : null,
