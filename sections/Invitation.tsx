...
export default function Invitation({ initialWedding }: { initialWedding?: Wedding }) {
  ...
  return (
    ...
    <section className={`relative min-h-[100svh] ${isSecondTheme ? "bg-[#f8f3ea] text-[#23382f]" : "bg-[#23382f] text-[#fffaf0]"}`}><Image src={activeWedding.images.hero} unoptimized={activeWedding.images.hero.startsWith("data:")} alt="Nadya dan Aldo berjalan di taman" fill priority sizes="100vw" className="object-cover object-center opacity-55" /><div className="absolute inset-0 bg-gradient-to-b from-[#183127]/25 via-[#193128]/20 to-[#183127]" /><div className="grain absolute inset-0 opacity-20" /><Container><div className="relative flex min-h-[100svh] flex-col items-center justify-between py-12 text-center"><p className="text-[10px] tracking-[.45em] uppercase text-[#ead6a1]">{isSecondTheme ? "The Wedding Of" : "Wedding Invitation"}</p><div><p className="mb-5 text-xs tracking-[.3em] uppercase text-white/80">Save the date</p><h1 className="script text-7xl leading-normal sm:text-8xl">{activeWedding.bride.nickName}</h1><p className="my-4 text-xl text-[#ead6a1]">&amp;</p><h1 className="script text-7xl leading-normal sm:text-8xl">{activeWedding.groom.nickName}</h1></div><div><p className="text-base font-light tracking-wide">{activeWedding.event.day}</p><a href="#mempelai" className="mt-6 inline-flex animate-bounce text-[#ead6a1]" aria-label="Lihat undangan"><ChevronDown /></a></div></div></Container></section>
    ...
    {preweddingImages.length > 0 && (
      <section className="bg-white py-24"><Container><SectionTitle eyebrow="Prewedding" title="Momen sebelum hari bahagia" /><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{preweddingImages.map((image, index) => <div key={`${image}-${index}`} className="relative overflow-hidden rounded-2xl border border-[#e5dccb] bg-[#f7f4ed]"><Image src={image} alt={`Prewedding ${index + 1}`} width={600} height={800} className="h-80 w-full object-cover" /></div>)}</div></Container></section>
    )}
