...
import { createCustomer, defaultCustomer, getCustomers, saveCustomers, type CustomerInvitation } from "@/lib/wedding-store";
...
export default function AdminDashboard() {
  ...
  const addCustomer = () => { const next = createCustomer(); setCustomers((items) => [...items, next]); setActiveId(next.id); setSaved(false); };
  const removeCustomer = () => { if (customers.length === 1) return; const next = customers.filter((customer) => customer.id !== active.id); setCustomers(next); setActiveId(next[0].id); setSaved(false); };
  const persist = () => { saveCustomers(customers); setSaved(true); window.setTimeout(() => setSaved(false), 2400); };
